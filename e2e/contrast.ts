 ...
 *    files the node under `incomplete`. Almost every surface here is a
 *    `color-mix(...)` tint or a translucent panel, so their text composites onto
 *    whatever is behind rather than a flat colour.
 *  - text faded by an ancestor's `opacity` — axe reads the declared `color`,
 *    which is not the colour that lands on screen. `.cl-hero-sub` used to fade
 *    `--muted` a further 85%, which is how it reached 2.96:1.
 *
 ...
 *    Paillier ciphertexts are 2048-bit integers printed in full, so at 380px
 *    much of them is clipped away. Content scrolled past the client box is not dimmed or partly
